...
def main():
    ...
    for mouse_id, mouse_data in tqdm(mice_data.items(), desc="Processing mice"):
        ...
        # Add child information to the tree table
        add_child(tree_table)

        # Get the per-mouse Q_intake (already set in load_trees if use_mouse_ventilation=True)
        mouse_Q_intake = mouse_data.get("Q_intake", args.Q_intake)

        # Propagate flow through the airway tree
        print(f"Propagating flow (Q={mouse_Q_intake*1e6:.2f} mL/s)...")
        tree_table.loc[tree_table['label'] == 1, 'Q'] = mouse_Q_intake
        propagate_flow(tree_table, parent_label=1, use_outlet=args.use_outlet_area)

        compute_generation(tree_table)

        # add generation information to mouse_data["deposition"]
        mouse_data["deposition"] = mouse_data["deposition"].merge(tree_table[['label','parent', 'name', 'child_1', 'child_2', 'child_3', 'generation']], on='label')
        
        # Save the tree with flow information
        ...
        # Save metadata
        with open(mouse_results_path / f"{mouse_id}_info.txt", "w") as info_file:
            info_file.write(mouse_data["info"])
        for line in mouse_data["info"].split("\n"):
            if 'Particle size:' in line:
                particle_size = float(line.split(':')[1].split()[0])
                break

        # For each impaction model, simulate and store results
        model_results = {}
        for impaction_model in args.impaction_models:
            print(f"Simulating deposition using {impaction_model} model for mouse {mouse_id}")
            # Create a DataFrame to store results for all particle sizes
            ...
            # Simulate deposition for each particle size
            for particle_diameter in tqdm(particle_sizes, desc="Particle sizes"):
                ...
                # Compute deposition probabilities
    
                compute_probabilities(particle_tree, particle_diameter, model=impaction_model)

                particle_tree.loc[particle_tree['label']==1, 'p_deposition_aggreg'] = 1-particle_tree.loc[particle_tree['label']==1, 'p_escape']
                particle_tree.loc[particle_tree['label']==1, 'p_escape_aggreg'] = particle_tree.loc[particle_tree['label']==1, 'p_escape']
                ...
                # Additional columns for specific mechanisms
        ...
        for i, particle_diameter in enumerate(particle_sizes):
            plt.figure(figsize=(10, 6))
            for impaction_model in args.impaction_models:
                print(f"Plotting results for {impaction_model} model and particle size {particle_diameter:.2e} m")
                deposition_results = model_results[impaction_model]
                generation_deposition = deposition_results.groupby('generation').sum()
                column_name = f"deposition_{particle_diameter:.2e}"
                plt.plot(generation_deposition.index, generation_deposition[column_name], label=f"{impaction_model}")
            # Ground truth
        ...
        # Combine results from all models and particle sizes into a single DataFrame
        combined_results = deposition_results[['label', 'parent', 'name', 'child_1', 'child_2', 'child_3', 'generation', 'radius', 'length', 'flow', 'theta', 'beta']].copy()
        for impaction_model, model_df in model_results.items():
            for particle_diameter in particle_sizes:
                ...
                # Only add impaction mechanism-specific columns per model
                mech = 'impaction'
                mech_col = f"{mech}_{particle_diameter:.2e}"
                combined_results[f"{impaction_model}_{mech_col}"] = model_df[mech_col]
                # Add sedimentation and diffusion only once (from the first model)
                if impaction_model == args.impaction_models[0]:
                    for mech in ['sedimentation', 'diffusion']:
                        mech_col = f"{mech}_{particle_diameter:.2e}"
                        combined_results[mech_col] = model_df[mech_col]
